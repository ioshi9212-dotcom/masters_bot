...
async def choose_client(message: Message, state: FSMContext, db) -> None:
    await state.clear()
...
async def edit_name_save(message: Message, state: FSMContext, db) -> None:
    ...
    if not text:
        await message.answer("Введите новое имя:")
        return
    await _save_client_field(message, db, "first_name", text, state)
...
async def edit_last_save(message: Message, state: FSMContext, db) -> None:
    ...
    if not text:
        await message.answer("Введите новую фамилию:")
        return
    await _save_client_field(message, db, "last_name", text, state)
...
async def edit_phone_save(message: Message, state: FSMContext, db) -> None:
    ...
    if not text:
        await message.answer("Введите новый телефон:")
        return
    await _save_client_field(message, db, "phone", text, state)
...
async def edit_birth_save(message: Message, state: FSMContext, db) -> None:
    ...
    if not text:
        await message.answer("Введите новую дату рождения (ДД.ММ):")
        return
    await _save_client_field(message, db, "birth_date", text, state)
...
async def pick_master(message: Message, state: FSMContext, db) -> None:
    master_id = _parse_token(_safe_text(message), "m")
    if not master_id:
...
async def pick_service(message: Message, state: FSMContext, db) -> None:
    service_id = _parse_token(_safe_text(message), "s")
    if not service_id:
    ...
    if not master_id:
        await state.set_state(ClientBookingState.pick_master)
        await message.answer("Сначала выберите мастера.")
        return
...
async def pick_date(message: Message, state: FSMContext, db) -> None:
    date_iso = _parse_date_token(_safe_text(message))
    if not date_iso:
    ...
    if not rows:
        await message.answer("На эту дату свободных окон уже нет. Выберите другую дату.")
        return
...
async def pick_time(message: Message, state: FSMContext, db) -> None:
    window_id = _parse_token(_safe_text(message), "w")
    if not window_id:
...
async def my_masters_pick(message: Message, state: FSMContext, db) -> None:
    text = _safe_text(message)
    master_id = _parse_token(text, "m")
    if not master_id:
        if text == "✉️ Написать мастеру":
            data = await state.get_data()
    ...
    try:
        ...
        cursor = await conn.execute("SELECT * FROM masters WHERE id = ?", (master_id,))
